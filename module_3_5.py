...
# 5. 4-й пункт можно выполнить только тогда, когда длина str_number больше 1,
...
def get_multiplied_digits(number):
    ...
    # print(f'str_number = {str_number}, first = {first}, len = {len(str_number)}')
    if len(str_number) > 1:
        # print(f'int(str_number[1:]) = {int(str_number[1:])}')
        return first * get_multiplied_digits(int(str_number[1:]))
    elif first == 0: # если number оканчивается на 0
        return 1
    else:
        return first
